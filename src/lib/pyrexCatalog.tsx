// PyrexSpinna Dynamic Lossless Catalog (Starts Empty for Solo Producer Ingestion)
export let pyrexCatalog: any[] = [];
...
export function uploadNewBeat(beatData: any) {
  ...
  if (!isHumanMade) {
    throw new Error("Upload rejected: PyrexSpinna strictly accepts human-produced beats only.");
  }

  const newTrack = {
    id: `ks-${Date.now()}`,
    title,
    producer: "PYREX", // Locked exclusively to your solo enterprise brand
    bpm,
    key,
    price,
    audioUrl,
