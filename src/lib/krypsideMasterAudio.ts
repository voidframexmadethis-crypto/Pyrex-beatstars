...
class KrypsideMasterAudioEngine {
  ...
  private setupAudioListeners() {
    ...
    this.audioElement.addEventListener('stalled', () => {
      console.warn("[KrypsideMasterAudio] Audio buffer stalled. Attempting recovery...");
      if (!this.hasRetried) {
    ...
    this.audioElement.addEventListener('error', (e) => {
      console.warn("[KrypsideMasterAudio] Audio element error:", e);
      if (!this.hasRetried) {
        this.hasRetried = true;
        setTimeout(() => {
          this.audioElement.load();
  ...
  public async play(target?: string | any): Promise<void> {
    ...
    if (typeof target === 'string') {
      ...
      trackId = target.id || '';
      if ((window as any).resolveKrypsideAudioStream) {
        url = (window as any).resolveKrypsideAudioStream(target);
      } else {
    ...
    if (this.audioElement && this.audioElement.src) {
      ...
      return this.audioElement.play().catch(async err => {
        console.warn("[KrypsideMasterAudio] Playback prevented or interrupted, attempting re-initialization:", err);
        if (this.audioElement) {
          try {
            if (url) {
              this.audioElement.src = url;
            ...
            console.error("[KrypsideMasterAudio] Re-initialization retry failed:", retryErr);
          }
        }
        throw err;
      });
